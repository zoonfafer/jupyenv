...
,
  ...
  mkJupyterlab =
    ...
    , # k: [ (k.python {}) k.bash ],
      ...
      runtimePackages ? [ ]
    , # runtime package available to all binaries
      flakes ? [ ]
    , # flakes where to detect custom kernels/extensions
    }:
    let
      allRuntimePackages =
        runtimePackages
  ...
  */
  ...
  eval = mkJupyterlabEval (_: { _module.check = false; });
